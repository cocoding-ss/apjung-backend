...
### 개발하고 서버에 반영하기
1. 작업하고 싶으신 이슈의 번호를 Branch로 만들어주세요. `issue#{이슈번호}` 만약 없다면 이슈를 만들어주세요. 이슈를 생성하실 때 우측의 Projects 설정을 꼭 해주세요.
2. 작업하신 브랜치를 Develop Branch로 `Pull Request`를 만들어주세요. 우측의 Linked Issue와 Request Reviewer를 꼭 설정해주세요.
3. PR은 자동으로 브랜치 테스트가 실행되며 머지가능한 브랜치인지 판별됩니다.
4. 머지가능한 상태가 되면 머지 버튼을 누르고 슬랙에서 배포가 정상적으로 이루어졌는지 확인해주세요. (보통 3분 정도의 시간이 소요됩니다)
